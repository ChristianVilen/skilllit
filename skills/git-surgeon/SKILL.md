...
## Handling untracked (new) files

git-surgeon operates on diff hunks. **Untracked files don't produce hunks** and
won't appear in `git-surgeon hunks` output. You must track them first.

### Simple case: commit new files alongside tracked changes

```bash
# Stage new files so they become tracked, then commit everything
git add src/newFile.ts src/newFile.test.ts
git-surgeon hunks --staged   # new files appear here
git-surgeon hunks             # tracked file changes appear here

# Commit staged + unstaged hunks together
git-surgeon commit <staged-ids> <unstaged-ids> -m "message"
```

### Commit-then-split: when new files need to go in a separate commit

When you need new files in one commit and modified tracked files in another,
**don't fight the index**. Commit everything together, then split:

```bash
# 1. Stage all files (new + modified)
git add src/newUtil.ts src/newUtil.test.ts
git-surgeon commit <all-hunk-ids> -m "temp: all changes"

# 2. List hunks in the combined commit
git-surgeon hunks --commit HEAD

# 3. Split into logical commits
git-surgeon split HEAD \
  --pick <new-file-hunks> -m "Add new utility" \
  --rest-message "Use utility in endpoints"
```

**Do NOT use** `git update-index --cacheinfo` with empty blobs — this creates
staged additions that block `git-surgeon commit` ("index already contains staged
changes" error). The commit-then-split pattern avoids this entirely.
